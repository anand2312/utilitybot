...
class Rolenames(commands.Cog):
    ...
    async def get_role_names(self) -> List[str]:
        ...
        async with self.bot.http_session.get(Rolenames.BASE_URL + "/rolenames", headers=self.headers) as res:
            if not res.status == 200:
                logger.error(f"Bot API returned non-200 status code: {res.status}\n{await res.json()}")
    ...
    async def add_role_name(self, name: str, by: int) -> None:
        ...
        async with self.bot.http_session.post(Rolenames.BASE_URL + "/rolenames", headers=self.headers, json=json) as res:
            if not res.status == 200:
                logger.error(f"Bot API returned non-200 status code: {res.status}\n{await res.json()}")
                res.raise_for_status()
    ...
    async def delete_role_name(self, name: str) -> None:
        ...
        json = {"rolename": name}
        async with self.bot.http_session.delete(Rolenames.BASE_URL + "/rolenames", headers=self.headers, json=json) as res:
            if not res.status == 200:
                logger.error(f"Bot API returned non-200 status code: {res.status}\n{await res.json()}")
                res.raise_for_status()
